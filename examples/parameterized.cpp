...
import std;
import boost.ut;
namespace boost::ext::ut
{

    namespace
    {
        template <std::floating_point F>
        std::string format_test_parameter(const std::complex<F> &arg, [[maybe_unused]] const int counter)
        {
...
int main()
{
    ...
    "views"_test = [](auto arg) { expect(arg > 0_i) << "all values greater than 0"; } | std::views::iota(1, 4);
#endif
    ...
    "parameterized test names"_test = []<class TArg>([[maybe_unused]] TArg arg) { expect(true); } |
