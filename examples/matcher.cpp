...
class matcher : public boost::ut::detail::op
{
    public:
        ...
        friend auto operator<<(std::ostream &os, const matcher &self) -> std::ostream &
        {
            return (os << self.str_);
        }
...
template <class... Ts> class any_of
{
    public:
...
int main()
{
    ...
    "matcher"_test = [] {
        constexpr auto is_between = [](auto lhs, auto rhs) {
            return [=](auto value) { return that % value >= lhs and that % value <= rhs; };
        };
        ...
        constexpr auto ends_with = [](const auto &arg, const auto &ext) {
            ...
            return matcher {std::equal(ext.rbegin(), ext.rend(), arg.rbegin()), str.str()};
        };
